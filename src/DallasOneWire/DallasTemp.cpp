...
//const std::list<DallasSensorData>&
int DallasTemp::readAllTempSerial(bool isCurr)
{
	if (isCurr)
	{
...
// select the device
			...
			if (_iface.OWReset())
			{
				...
				if (_iface.OWBlock(sendpacket, sendlen))
				{
					...
					mainBuff.writeTemp(DallasSensorData(_iface.ROM_NO, temp));
...
#ifdef LEVEL_INFO
		char buf[20];
		sprintf(buf, "%d sensors readed", mainBuff.size());
		INFO(buf);
#endif
	}
	return mainBuff.size();
